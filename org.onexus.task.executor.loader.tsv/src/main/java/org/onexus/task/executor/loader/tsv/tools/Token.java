 ...
 *
 */
package org.onexus.task.executor.loader.tsv.tools;

/**
 * This class provides convenience operations for handling so-called 'Tokens'.
 * Tokens in this project are nothing else than UTF8 encoded strings, stored in
 * a byte array.
 * 
...
public final class Token {
    ...
    /**
     * Converts a string to a byte array. All strings should be converted by
     * this function to guarantee a consistent character conversion.
     * 
     * @param s
	...
	// no UTF8 (string constant..) & no special characters: return input
	// string
	if (enc == UTF8 || ascii(s))
	    return s;
	// convert to utf8
	...
	if (c == 14)
	    return (v & 0x0F) << 12 | (t[++l] & 0x3F) << 6 | t[++l] & 0x3F;
	// 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
	return (v & 0x07) << 18 | (t[++l] & 0x3F) << 12 | (t[++l] & 0x3F) << 6
		| t[++l] & 0x3F;
    }
	...
	int l = 0;
	for (int t = 0; t < text.length; t += cl(text[t]))
	    l++;
	return l;
    }
    ...
    /**
     * Creates a byte array representation from the specified long value, using
    ...
    /** US charset. */
    public static final java.text.DecimalFormatSymbols LOC = new java.text.DecimalFormatSymbols(
	    java.util.Locale.US);
    /** Scientific double output. */
    private static final java.text.DecimalFormat SD = new java.text.DecimalFormat(
	    "0.0################E0##", LOC);
    /** Decimal double output. */
    private static final java.text.DecimalFormat DD = new java.text.DecimalFormat(
	    "#####0.0################", LOC);
    /** Scientific float output. */
    private static final java.text.DecimalFormat SF = new java.text.DecimalFormat(
	    "0.0####E0##", LOC);
	...
	return chopNumber(token(a >= 1e-6 && a < 1e6 ? DD.format(d) : SD
		.format(d)));
    }
	...
	if (d == -1 / 0d)
	    return NINF;
	if (d == 0)
	    return 1 / d > 0 ? ZERO : MZERO;
	// TODO if(d != d) return NAN;
	final double a = Math.abs(d);
	if (a < 1e6) {
	    final int i = (int) d;
	    if (i == d)
    ...
    private static final byte[][] FLTSTR = { token("1.0E17"), token("1.0E15"),
	    token("1.0E13"), token("1.0E11"), token("-1.0E17"),
	    token("-1.0E15"), token("-1.0E13"), token("-1.0E11") };

    /**
     * Converts the specified token into a double value. {@link Double#NaN} is
    ...
    /**
     * Converts the specified string into a double value. {@link Double#NaN} is
     * returned when the input is invalid.
     * 
     * @param to
     *            character array to be converted
     * @return converted double value
    ...
    /**
     * Converts the specified string into an long value. {@link Long#MIN_VALUE}
    ...
    /**
     * Converts the specified token into an long value. {@link Long#MIN_VALUE}
     * is returned when the input is invalid.
     * 
     * @param to
     *            character array to be converted
     ...
     * {@link Integer#MIN_VALUE} is returned if non-digits are found or if the
     * input is longer than nine characters.
     * 
     * @param to
     *            character array to be converted
     * @return converted integer value
    ...
    private static final boolean[] LOD = { true, true, true, true, true, true,
	...
	return ch >= 'a' && ch <= 'z' ? ch - 32 : ch > 0x7F ? Character
		.toUpperCase(ch) : ch;
    }
	...
	return ch >= 'A' && ch <= 'Z' ? ch | 0x20 : ch > 0x7F ? Character
		.toLowerCase(ch) : ch;
    }
    ...
    public static final byte[] NORM = { 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'C',
	    'E', 'E', 'E', 'E', 'I', 'I', 'I', 'I', 'D', 'N', 'O', 'O', 'O',
	    'O', 'O', ' ', 'O', 'U', 'U', 'U', 'U', 'Y', 'D', 'S', 'a', 'a',
