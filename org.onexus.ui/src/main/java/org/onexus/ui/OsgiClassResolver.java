...
public final class OsgiClassResolver implements IClassResolver {
    /**
    ...
    public OsgiClassResolver(ClassLoader... loader) {
        super();
        this.loader = loader;
    }
        ...
        if (clazz == null) {
            if (classname.equals("byte")) {
                ...
                synchronized (classes) {
                    for (int i = 0; i < loader.length && clazz == null; i++) {
                        try {
                            clazz = Class.forName(classname, false, loader[i]);
                        } catch (ClassNotFoundException e) {
                            if (i + 1 == loader.length) {
                                throw e;
                            }
                        }
                    }
