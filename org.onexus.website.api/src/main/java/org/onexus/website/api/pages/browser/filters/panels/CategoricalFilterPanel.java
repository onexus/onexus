...
public abstract class CategoricalFilterPanel extends AbstractFilterPanel<String> {
    ...
    public CategoricalFilterPanel(String id, FieldHeader fieldHeader, List<String> values) {
        super(id, fieldHeader, new FilterOption<String>(EqualOperation.INSTANCE, values.get(0)), OPERATIONS);
        this.values = values;
    }

    @Override
    protected FormComponent<String> createValueFormComponent(String componentId) {
        return new DropDownChoice<String>(componentId, values).setNullValid(false);
    }
}
